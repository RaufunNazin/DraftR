...
export async function getAvailableUsers(tournamentId: string) {
  ...
  if (!session || !session.user) {
    return { success: false, error: "You must be logged in to view available users" }
  }
 ...
 */
export async function assignCaptain(tournamentId: string, userId: string, tier: Tier, role: PlayerRole) {
  const session = await auth()

  if (!session || !session.user) {
    return { success: false, error: "You must be logged in to assign captains" }
  }
  ...
  if (!currentUser || (currentUser.role !== "ADMIN" && tournament.hostId !== session.user.id)) {
    return { success: false, error: "Only admins or the tournament host can assign captains" }
  }
  ...
  try {
    ...
    if (existingCaptain) {
      return { success: false, error: "User is already a captain in this tournament" }
    }
...
export async function removeCaptain(captainId: string) {
  ...
  if (!session || !session.user) {
    return { success: false, error: "You must be logged in to remove captains" }
  }
  ...
  try {
    ...
    if (!user || (user.role !== "ADMIN" && captain.tournament.hostId !== session.user.id)) {
      return { success: false, error: "Only admins or the tournament host can remove captains" }
    }

    // Check if captain has players
    if (captain.players.length > 0) {
      return { success: false, error: "Cannot remove a captain that has players assigned" }
    }
