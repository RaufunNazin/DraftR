      ...
      connect: () => {
        ...
        socket.on(
          ...
          (data: {
            ...
            set((state) => {
              if (!state.auctionState) return { auctionState: null, localAuctionState: null }
        ...
        socket.on("auction:complete", (data: AuctionHistoryItem | { isActive: boolean }) => {
          set((state) => {
            ...
            toast({
              ...
              description: `${data.playerName} was drafted by ${data.captainName} for ${data.finalBid} credits`,
              variant: "default",
            })
...
export function useAuctionState() {
  return useSocketStore((state) => state.localAuctionState || state.auctionState)
}
