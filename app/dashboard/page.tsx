            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {tournaments.length === 0 ? (
                <div className="col-span-full text-center py-12">
                  ...
                  <p className="text-muted-foreground mb-6">You haven't joined any tournaments yet.</p>
                  <Button
                    variant="outline"
                    onClick={() =>
                      document.querySelector('[data-value="join"]')?.dispatchEvent(new MouseEvent("click"))
                    }
                  >
                    Join a Tournament
                  </Button>
                ...
                tournaments.map((tournament) => (
                  <Card key={tournament.id} className="border-purple-500/20 shadow-lg hover:shadow-purple-500/5">
                    <CardHeader>
                    ...
                    <CardContent>
                      <div className="flex items-center gap-2 mb-4">
                        ...
                        <span className="text-sm text-muted-foreground">
                          {tournament.captains?.length || 0} Captains • {tournament.players?.length || 0} Players
                        </span>
                      </div>
                      <div className="bg-secondary/30 rounded-md px-3 py-2 text-center">
                        <p className="text-xs text-muted-foreground mb-1">Tournament Code</p>
                        <p className="font-mono font-bold tracking-wider">{tournament.code}</p>
                      </div>
                    </CardContent>
                    <CardFooter>
                      <Button className="w-full" onClick={() => router.push(`/auction?tournament=${tournament.code}`)}>
                        {tournament.status === "ACTIVE" ? "Join Auction" : "View Tournament"}
                      </Button>
                    </CardFooter>
                  </Card>
                ))
              ...
              {session?.user?.role === "ADMIN" && (
                <Card className="border-dashed border-muted hover:border-muted-foreground/50 transition-colors flex flex-col items-center justify-center h-[300px]">
                  ...
                  >
                    ...
                    <span className="text-muted-foreground font-normal">Create New Tournament</span>
                  </Button>
                </Card>
              )}
            </div>
