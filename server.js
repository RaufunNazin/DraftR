    ...
    io.on("connection", (socket) => {
      ...
      socket.on("join-tournament", async (tournamentCode) => {
        try {
          ...
          if (!auctionState) {
            ...
            if (!dev) {
              redisClient.publish("tournament-cache-update", JSON.stringify({ tournamentCode, data: auctionState }))
            }
          }
      ...
      socket.on("auction:bid", async (data) => {
        try {
          ...
          if (captain.tier === auctionState.currentPlayer.tier) {
            socket.emit("error", { message: "Cannot bid on your own tier" })
            return
          }

          if (captain.pickedTiers.some((pt) => pt.tier === auctionState.currentPlayer.tier)) {
      ...
      socket.on("auction:skip", async (data) => {
        try {
          ...
          console.log(`Skip votes: ${updatedSkipVotes.length}/${skipThreshold} needed`)
        ...
        try {
          ...
          if (isPaused) {
            ...
            if (pausedTimers.has(auctionState.auctionId)) {
              ...
              startAuctionTimer(auctionState.auctionId, tournamentCode, io, pausedState.timer)
      ...
      socket.on("disconnect", () => {
        console.log(`Client disconnected: ${socket.id} from worker ${process.pid}`)
      })
    })
...
async function selectNextPlayer(auctionId, tournamentCode, io) {
  try {
    ...
    // Broadcast the new player immediately
    io.to(`tournament:${tournamentCode}`).emit("auction:player", {
      player: {
        id: nextPlayer.id,
    ...
    // Then persist to database immediately
    await prisma.auction.update({
      where: { id: auctionId },
      data: {
        currentPlayerId: nextPlayer.id,
...
async function startAuctionTimer(auctionId, tournamentCode, io, initialTimer = null) {
  ...
  const runTimer = async () => {
    try {
      ...
      let currentTimer = initialTimer !== null ? initialTimer : auctionState.timer
      ...
      // Broadcast timer update
      io.to(`tournament:${tournamentCode}`).emit("auction:timer", { timer: currentTimer })
      ...
      if (currentTimer <= 0) {
        ...
        if (auctionState.currentBidder) {
          const highestBid = {
            ...
            captainId: auctionState.currentBidder.id,
          }

          // Find the captain in the cached state
          const captain = auctionState.captains.find((c) => c.id === highestBid.captainId)
          ...
          if (captain) {
            ...
            const updatedCaptains = auctionState.captains.map((c) => {
              if (c.id === captain.id) {
                return {
                  ...c,
                  credits: c.credits - highestBid.amount,
                  pickedTiers: [...c.pickedTiers, { tier: auctionState.currentPlayer.tier }],
                  pickedPlayers: [...(c.pickedPlayers || []), auctionState.currentPlayer],
                }
              }
              return c
            })

            const updatedPlayers = auctionState.players.filter((p) => p.id !== auctionState.currentPlayer.id)
            ...
            const updatedAuctionState = {
              ...
              history: [historyEntry, ...auctionState.history],
              currentPlayer: null,
              currentBid: 0,
              currentBidder: null,
            }
            ...
            // Broadcast auction completion immediately
            io.to(`tournament:${tournamentCode}`).emit("auction:complete", historyEntry)
...
async function shutdown() {
  ...
  await Promise.all([prisma.$disconnect(), redisClient.quit(), redisPub.quit(), redisSub.quit()])
